...
type UpsertElem (key :: Symbol) (t :: k) (ts :: [(Symbol, k)]) =
    FromMaybe ('(key, t) ': ts) =<< Map (Flipper SetIndex '(key, t) ts) =<< FindIndex (TyEq key <=< Fst) ts

-- All credit goes to the Solutions appendix.
data Flipper
    :: (a -> b -> c -> Exp r)
    -> b
    -> c
...
type instance Eval (Flipper f b c a) =
    Eval (f a b c)
